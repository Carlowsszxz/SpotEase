...
-- Convert existing RFID timestamps from naive timestamps to UTC-aware timestamptz.
-- Assumes current stored values were captured as UTC-like wall clock values.
ALTER TABLE public.rfid_scans
...
-- Prefer payload.tag_uid -> user_rfid_tags.tag_uid_norm, with hash fallback.
UPDATE public.rfid_scans s
SET user_id = t.user_id
FROM public.user_rfid_tags t
WHERE s.user_id IS NULL
  AND t.is_active = true
  AND upper(regexp_replace(COALESCE(s.payload->>'tag_uid', ''), '\\s+', '', 'g')) = t.tag_uid_norm;
