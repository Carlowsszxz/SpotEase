...
(async function(){
    ...
    function renderActivity(items){
        ...
        if(!items || items.length === 0){
            if(noActivityEl){
                noActivityEl.textContent = 'No RFID taps yet.';
            }
        ...
        visible.forEach(function(it){
            ...
            var who = (it.resource && (it.resource.name || it.resource.location))
                ? (it.resource.name || 'Unknown')
                : (it.resource_id || 'Unknown');
            var loc = it.resource && it.resource.location ? it.resource.location : '';

            left.innerHTML = '<strong>RFID Tap</strong> · ' + escapeHtml(who)
                + (loc ? '<div class="meta">' + escapeHtml(loc) + '</div>' : '');

            right.innerHTML = '<span class="meta">' + escapeHtml(formatWhen(it.scanned_at)) + '</span>';
    ...
    async function loadRecentActivity(){
        try{
            ...
            if(!currentUser){
                renderActivity([]);
                if(noActivityEl) noActivityEl.textContent = 'Not logged in.';
                return;
            }

            async function sha256Hex(text){
                var bytes = new TextEncoder().encode(String(text || '').trim().toUpperCase());
            ...
            // First, get the user's registered RFID tags
            var userTagsResult = await supabase
                .from('user_rfid_tags')
                .select('tag_uid_norm, tag_uid')
                .eq('user_id', currentUser.id);

            var userTags = userTagsResult.data || [];
            if(!userTags.length){
                renderActivity([]);
                if(noActivityEl) noActivityEl.textContent = 'No RFID tags registered.';
                return;
            }

            var tagHashes = await Promise.all(userTags.map(function(t){ return sha256Hex(t.tag_uid_norm || t.tag_uid); }));

            tagHashes = tagHashes.filter(function(v){ return !!v; });
            ...
            // Then get RFID scans for those tags
            var rfidResult = await supabase
                .from('rfid_scans')
                .select('scanned_at, resource_id, tag_hash')
                .in('tag_hash', tagHashes)
                .order('scanned_at', { ascending: false })
                .limit(25);

            var rfidScans = rfidResult.data || [];
            var scansErr = rfidResult.error;

            if(scansErr){
                renderActivity([]);
                if(noActivityEl) noActivityEl.textContent = 'RFID activity is not available.';
                return;
            }

            var resourceIds = Array.from(new Set((rfidScans || []).map(function(s){ return s.resource_id; }).filter(Boolean)));
            ...
            var hydrated = (rfidScans || []).map(function(s){
                return {
                    id: s.resource_id + '_' + s.scanned_at,
                    resource_id: s.resource_id,
                    scanned_at: s.scanned_at,
                    resource: s.resource_id ? resourcesById.get(s.resource_id) : null
                };
            });
            ...
            renderActivity([]);
            if(noActivityEl){
                noActivityEl.textContent = 'RFID activity is not available.';
            }
            applyActivityPanelVisibility();
        }
    }
    ...
    if(activityListEl || noActivityEl){
            // subscribeActivityRealtime();  // RFID scans are user-scoped, no need for realtime
    }
