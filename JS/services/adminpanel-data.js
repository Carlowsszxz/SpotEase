...
export async function fetchAssignableUsersForAdminUi(supabase) {
  try {
    const { data, error } = await supabase.rpc('admin_list_assignable_users');
    if (!error && data) return data;
  } catch (e) {
    // Ignore and fall back.
  }
  ...
  const assignedUserIds = new Set((tagsResult.data || []).map((row) => row.user_id).filter(Boolean))
  return (usersResult.data || []).filter((user) => !assignedUserIds.has(user.id))
}
...
export async function fetchRecentUnassignedRfidTaps(supabase, limit) {
  const query = supabase
    .from('rfid_scans')
    .select('id, tag_hash, scanned_at, sensor_id, resource_id, payload, user_id')
    .is('user_id', null)
