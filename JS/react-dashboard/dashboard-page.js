import { createClient as e } from "https://cdn.jsdelivr.net/npm/@supabase/supabase-js/+esm";
//#region \0rolldown/runtime.js
var t = Object.create, n = Object.defineProperty, r = Object.getOwnPropertyDescriptor, i = Object.getOwnPropertyNames, a = Object.getPrototypeOf, o = Object.prototype.hasOwnProperty, s = (e, t) => () => (e && (t = e(e = 0)), t), c = (e, t) => () => (t || e((t = { exports: {} }).exports, t), t.exports), l = (e, t, a, s) => {
	...
	return e;
}, u = (e, r, i) => (i = e == null ? {} : t(a(e)), l(r || !e || !e.__esModule ? n(i, "default", {
	value: e,
	enumerable: !0
}) : i, e)), d = /* @__PURE__ */ c(((e) => {
...
//#endregion
//#region JS/supabase-auth.js
async function y(e) {
	if (!e) return !1;
	let t = e.email || e.user_metadata && e.user_metadata.email || null, n = e.user_metadata?.full_name || e.user_metadata?.name || t || null;
	try {
		let { data: r, error: i } = await T.from("users").select("id, name, email, role").eq("id", e.id);
		if (i && console.warn("User sync select failed", i), r && r.length > 0) {
			let i = r[0], a = { updated_at: (/* @__PURE__ */ new Date()).toISOString() }, o = i && i.name != null ? String(i.name).trim() : "", s = i && i.email != null ? String(i.email).trim() : "", c = t ? String(t).trim() : "", l = n ? String(n).trim() : "";
			if (c && c !== s && (a.email = c), l && (!o || s && o === s) && l !== o && (a.name = l), Object.prototype.hasOwnProperty.call(a, "name") || Object.prototype.hasOwnProperty.call(a, "email")) {
				let { error: t } = await T.from("users").update(a).eq("id", e.id);
				t && console.warn("User sync update failed", t);
			}
			return !0;
		}
		let a = (/* @__PURE__ */ new Date()).toISOString(), o = {
			...
			created_at: a,
			updated_at: a
		}, { error: s } = await T.from("users").insert([o]);
		if (s) {
			console.warn("User sync insert failed", s);
			try {
				let { data: t } = await T.from("users").select("id").eq("id", e.id);
		...
		return console.warn("syncUserRecord failed", e), !1;
	}
}
async function b(e, { retries: t = 8, delayMs: n = 250 } = {}) {
	if (!e) return !1;
	for (let r = 0; r < t; r++) {
		if (await y(e) || await te(e)) return !0;
		await new Promise((e) => setTimeout(e, n));
	}
	return !1;
}
function x(e, t = !1) {
	ae && (ae.textContent = e, ae.style.color = t ? "crimson" : "green");
}
async function ee() {
	try {
		let e = window.location.origin + "/FrameRegister.html", { error: t } = await T.auth.signInWithOAuth({
			provider: "google",
			options: { redirectTo: e }
		});
		if (t) throw t;
	} catch (e) {
		x("Google sign-in error: " + (e.message || e), !0), console.error("signInWithGoogle error", e);
	}
}
async function S(e) {
	if (oe) return;
	oe = !0;
	let t = e.user || e && e.user;
	if (!t) return;
	let n = t.email || t.user_metadata && t.user_metadata.email, r = t.identities && t.identities.some((e) => e.provider === "email"), i = t.identities && t.identities.some((e) => e.provider !== "email"), a = !1;
	try {
		a = window.location.pathname.endsWith("FrameRegister.html") || !!document.getElementById("register-message"), window.location.pathname.endsWith("FrameEmailConfirm.html");
	} catch {
		a = !1;
	}
	if (a && r && !i && !t.email_confirmed_at) {
		await T.auth.signOut(), x("Please check your email to confirm your account.", !1);
		return;
	}
	let o = await y(t);
	if (a) {
		x(o ? "Account created successfully! Redirecting..." : "Signed in. Redirecting...", !1), setTimeout(() => {
			window.location.href = "FrameDashboard.html";
		}, 1500);
		return;
	}
	x("Signed in as " + (n || t.id), !1), console.log("Supabase session:", e);
}
async function C() {
...
async function te(e) {
	if (!e) return !1;
	try {
		let { data: t, error: n } = await T.from("users").select("id").eq("id", e.id);
...
async function ne() {
	let e = await C();
	return e ? !await te(e) && (await b(e), !await te(e)) ? (await T.auth.signOut(), window.location.href = "/FrameLogin.html", !1) : !0 : (window.location.href = "/FrameLogin.html", !1);
}
var re, w, T, ie, ae, oe, se = s((() => {
	re = "https://xsgymjzkuiohsqalrqkw.supabase.co", w = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InhzZ3ltanprdWlvaHNxYWxycWt3Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzI2Mjk2NjMsImV4cCI6MjA4ODIwNTY2M30.CKZoZbIu0MDZplXl_8Qf-K6n5nFaPMvp4ZHp7TcdIkM", T = e(re, w), ie = document.getElementById("gSignUpBtn"), ae = document.getElementById("register-message"), ie && ie.addEventListener("click", ee), oe = !1, (async function() {
		try {
			if (typeof T.auth.getSessionFromUrl == "function") try {
				let { data: e, error: t } = await T.auth.getSessionFromUrl({ storeSession: !0 });
				t && !/No URL parameters found/.test(t.message || "") && console.warn("getSessionFromUrl warning", t);
			} catch (e) {
				console.warn("getSessionFromUrl failed", e);
			}
			let { data: e } = await T.auth.getSession(), t = e.session;
			t && S(t);
		} catch (e) {
			console.error("OAuth redirect handling failed", e);
		}
	})(), T.auth.onAuthStateChange((e, t) => {
		t ? S(t) : e === "SIGNED_OUT" && x("Signed out", !1);
	});
}));
//#endregion
//#region JS/services/dashboard-data.js
async function ce(e, t) {
	...
	try {
		...
		if (!r) return n;
		try {
			let { data: t, error: i } = await e.from("users").select("name").eq("id", r.id).single();
			n = !i && t && t.name ? String(t.name || "").trim() || "User" : r.user_metadata && (r.user_metadata.full_name || r.user_metadata.name) || r.email || "User";
		} catch {
			n = r.user_metadata && (r.user_metadata.full_name || r.user_metadata.name) || r.email || "User";
		}
	} catch {}
	return n;
}
async function le(e) {
	let { data: t, error: n } = await e.from("resources").select("id,name,location,capacity,current_occupancy,is_active").or("is_active.is.null,is_active.eq.true").order("name", { ascending: !0 });
	if (n) throw n;
	return t || [];
}
async function E(e, t) {
	if (!t || !t.length) return [];
	let { data: n, error: r } = await e.from("resources").select("id,name,location").in("id", t);
	if (r) throw r;
	return n || [];
}
async function D(e, t) {
	let n = e.from("announcements").select("id,title,message,start_at,end_at,created_at").eq("is_active", !0).order("created_at", { ascending: !1 });
	typeof t == "number" && t > 0 && n.limit(t);
	let { data: r, error: i } = await n;
	if (i) throw i;
	let a = Date.now();
	return (r || []).filter(function(e) {
		var t = !e.start_at || !isNaN(new Date(e.start_at).getTime()) && new Date(e.start_at).getTime() <= a, n = !e.end_at || !isNaN(new Date(e.end_at).getTime()) && new Date(e.end_at).getTime() >= a;
		return t && n;
	});
}
var ue = s((() => {})), de = /* @__PURE__ */ c(((e) => {
	...
	var e = /* @__PURE__ */ u(f()), t = v();
	se(), ue();
	var n = fe(), r = 6;
	async function i(e) {
		let t = new TextEncoder().encode(String(e || "").trim().toUpperCase()), n = await crypto.subtle.digest("SHA-256", t);
		return Array.from(new Uint8Array(n)).map((e) => e.toString(16).padStart(2, "0")).join("");
	}
	function a(e, t) {
		let n = Number(e || 0), r = Number(t || 0);
		return !r || r <= 0 ? "Unknown" : n >= r ? "Full" : n / r >= .8 ? "Near full" : "Available";
	}
	function o(e) {
		if (!e) return 99;
		let t = a(e.current_occupancy, e.capacity);
		return t === "Full" ? 0 : t === "Near full" ? 1 : t === "Available" ? 2 : 3;
	}
	function s(e) {
		if (!e) return "";
		let t = new Date(e);
		return Number.isNaN(t.getTime()) ? "" : t.toLocaleString("en-PH", { timeZone: "Asia/Manila" });
	}
	function c(e, t) {
		let n = s(e), r = s(t);
		return n && r ? `${n} to ${r}` : n ? `starts ${n}` : r ? `until ${r}` : "";
	}
	function l() {
		return [
			{
	...
	function d({ digit: t }) {
		let [r, i] = (0, e.useState)(() => Math.max(0, Math.min(9, Number(t) || 0))), [a, o] = (0, e.useState)(!0), s = (0, e.useRef)(Math.max(0, Math.min(9, Number(t) || 0)));
		return (0, e.useEffect)(() => {
			let e = Math.max(0, Math.min(9, Number(t) || 0)), n = s.current, r = null, a = null, c = null;
			return n === e ? (o(!1), i(e), r = window.requestAnimationFrame(() => {
				o(!0);
			}), () => {
				r && window.cancelAnimationFrame(r);
			}) : (o(!1), i(n), r = window.requestAnimationFrame(() => {
				a = window.requestAnimationFrame(() => {
					let t = e < n ? e + 10 : e;
					o(!0), i(t), s.current = e, t >= 10 && (c = window.setTimeout(() => {
						o(!1), i(e), window.requestAnimationFrame(() => {
							o(!0);
						});
					}, 540));
				});
			}), () => {
				r && window.cancelAnimationFrame(r), a && window.cancelAnimationFrame(a), c && window.clearTimeout(c);
			});
		}, [t]), /* @__PURE__ */ (0, n.jsx)("span", {
			className: "roll-col",
			"data-digit": t,
			children: /* @__PURE__ */ (0, n.jsx)("span", {
				...
				style: {
					...
					transition: a ? "transform 520ms cubic-bezier(.22, 1, .36, 1)" : "none"
				},
				children: Array.from({ length: 20 }, (e, t) => /* @__PURE__ */ (0, n.jsx)("span", {
	...
	function p({ value: t }) {
		let r = Number(t || 0);
		Number.isFinite(r) || (r = 0), r = Math.max(0, Math.floor(r));
		let i = String(r).split("").map((e) => Number(e)), [a, o] = (0, e.useState)(() => Math.max(2, i.length));
		(0, e.useEffect)(() => {
			o((e) => Math.max(e, 2, i.length));
		}, [i.length]);
		let s = (0, e.useMemo)(() => {
			let e = Math.max(0, a - i.length);
			return [...Array.from({ length: e }, () => null), ...i];
		}, [i, a]);
		return /* @__PURE__ */ (0, n.jsx)("div", {
			className: "roll-number",
			"aria-label": String(r),
			children: s.map((e, t) => /* @__PURE__ */ (0, n.jsx)("span", {
				className: `roll-digit-slot ${e == null ? "is-leading-empty" : ""}`,
				children: /* @__PURE__ */ (0, n.jsx)(d, { digit: e ?? 0 })
			}, `slot-${t}`))
		});
	}
	function m() {
		let [t, u] = (0, e.useState)(!1), [d, f] = (0, e.useState)(!0), [m, h] = (0, e.useState)("User"), [g, _] = (0, e.useState)({
			text: "",
			error: !1
		}), [v, y] = (0, e.useState)(""), [b, x] = (0, e.useState)(/* @__PURE__ */ new Map()), [ee, S] = (0, e.useState)(""), [te, re] = (0, e.useState)([]), [w, ie] = (0, e.useState)([]), [ae, oe] = (0, e.useState)(""), [se, ue] = (0, e.useState)(!1), [de, fe] = (0, e.useState)(!1), pe = (0, e.useRef)(null), O = (0, e.useRef)(null), k = (0, e.useRef)(null), me = (0, e.useRef)(null), he = (0, e.useRef)(null), ge = (0, e.useRef)(null), _e = (0, e.useRef)(!1), ve = (0, e.useMemo)(() => Array.from(b.values()).filter((e) => e && (e.is_active === !0 || e.is_active == null)).sort((e, t) => {
			let n = o(e) - o(t);
			return n === 0 ? String(e.name || "").localeCompare(String(t.name || ""), void 0, { sensitivity: "base" }) : n;
		}), [b]), ye = (0, e.useMemo)(() => {
			let e = 0, t = 0, n = 0, r = 0;
			return ve.forEach((i) => {
				let a = Number(i.current_occupancy || 0), o = Number(i.capacity || 0);
				Number.isFinite(a) || (a = 0), Number.isFinite(o) || (o = 0), e += Math.max(0, a), t += 1, o > 0 && (a >= o ? r += 1 : a / o >= .8 && (n += 1));
			}), {
				peopleInside: Math.max(0, Math.floor(e)),
				activeSpaces: t,
				nearFull: n,
				full: r
			};
			...
			let e = [];
			return ye.full > 0 && e.push({
				id: "full",
				type: "system",
				message: `${ye.full} space(s) are full right now`
			}), ye.nearFull > 0 && e.push({
				id: "near-full",
				type: "system",
				message: `${ye.nearFull} space(s) are near full`
			}), ye.activeSpaces === 0 && e.push({
				id: "none-active",
				type: "system",
				message: "No active spaces found in resources"
			}), e;
		}, [ye]), xe = (0, e.useMemo)(() => [...te, ...be], [te, be]), Se = (0, e.useMemo)(() => de ? w : w.slice(0, r), [de, w]), Ce = (0, e.useCallback)(async () => {
			if (!_e.current) {
				_e.current = !0;
				try {
					let e = await le(T), t = /* @__PURE__ */ new Map();
					(e || []).forEach((e) => {
						e?.id && t.set(e.id, e);
					}), x(t), S("");
				} catch (e) {
					console.error("loadOccupancy error", e), S("Failed to load live occupancy. Showing sample data below."), _({
						text: `Failed to load resources: ${e?.message || e}`,
						error: !0
					});
				} finally {
					_e.current = !1;
		...
		}, []), we = (0, e.useCallback)(async () => {
			try {
				re((await D(T, 8) || []).map((e) => ({
					id: e.id,
					type: "announcement",
					title: String(e.title || "Notice"),
					message: String(e.message || ""),
					when: c(e.start_at, e.end_at)
				})));
			} catch (e) {
				console.warn("loadAnnouncementAlerts error", e), re([]);
			}
		}, []), Te = (0, e.useCallback)(async () => {
			try {
				let { data: e } = await T.auth.getUser(), t = e?.user;
				if (!t) {
					ie([]), oe("Not logged in.");
					return;
				}
				let { data: n } = await T.from("user_rfid_tags").select("tag_uid_norm, tag_uid").eq("user_id", t.id);
				if (!n || n.length === 0) {
					ie([]), oe("No RFID tags registered.");
					return;
				}
				let r = await Promise.all(n.map((e) => e.tag_uid_norm || e.tag_uid).filter(Boolean).map((e) => i(e)));
				if (!r.length) {
					ie([]), oe("No RFID tags registered.");
					return;
				}
				let { data: a, error: o } = await T.from("rfid_scans").select("scanned_at, resource_id, tag_hash").in("tag_hash", r).order("scanned_at", { ascending: !1 }).limit(25);
				if (o) {
					ie([]), oe("RFID activity is not available.");
					return;
				}
				let s = Array.from(new Set((a || []).map((e) => e.resource_id).filter(Boolean))), c = /* @__PURE__ */ new Map();
				if (s.length > 0) try {
					(await E(T, s) || []).forEach((e) => {
						c.set(e.id, e);
					});
				} catch {}
				ie((a || []).map((e) => ({
					id: e.resource_id + "_" + e.scanned_at,
					resource_id: e.resource_id,
					scanned_at: e.scanned_at,
					resource: e.resource_id ? c.get(e.resource_id) : null
				}))), oe("");
			} catch (e) {
				console.warn("loadRecentActivity error", e), ie([]), oe("RFID activity is not available.");
			}
			...
			try {
				await Promise.all([
					...
					text: "",
					error: !1
				}), y(`Updated: ${(/* @__PURE__ */ new Date()).toLocaleString()}`);
			} catch (e) {
				console.error("loadAll error", e), _({
					text: `Failed to load dashboard: ${e?.message || e}`,
					error: !0
				});
			} finally {
				f(!1);
			...
			Ce,
			Te
		]), De = (0, e.useCallback)((e = 1e3) => {
			if (k.current) return;
			let t = async () => {
				await Ce(), k.current = window.setTimeout(t, Math.max(250, e));
			};
			t();
		}, [Ce]), Oe = (0, e.useCallback)(() => {
			k.current &&= (clearTimeout(k.current), null);
		}, []), ke = (0, e.useCallback)((e = 15e3) => {
			if (me.current) return;
			let t = async () => {
				await Te(), me.current = window.setTimeout(t, Math.max(2e3, e));
			};
			t();
		}, [Te]), Ae = (0, e.useCallback)(() => {
			me.current &&= (clearTimeout(me.current), null);
		}, []), je = (0, e.useCallback)(() => {
			if (pe.current) return;
			let e = T.channel("dashboard-resources-live-react").on("postgres_changes", {
				event: "*",
				schema: "public",
				table: "resources"
			}, (e) => {
			...
			try {
				e.subscribe((e) => {
					e === "SUBSCRIBED" ? (De(1e3), Ce()) : (e === "CHANNEL_ERROR" || e === "TIMED_OUT" || e === "CLOSED") && De(1e3);
				});
			} catch {
				De(1e3);
			}
		}, [Ce, De]), Me = (0, e.useCallback)(() => {
			if (!O.current) try {
				let e = T.channel("dashboard-occupancy-events-react").on("postgres_changes", {
					event: "INSERT",
					schema: "public",
					...
					try {
						let t = e?.new?.id;
						if (!t || ge.current && t === ge.current) return;
						Te();
					} catch {}
				});
				O.current = e, e.subscribe((e) => {
					e === "SUBSCRIBED" && ke(15e3);
				});
			} catch {
				ke(15e3);
			}
		...
		return (0, e.useEffect)(() => {
			...
			return (async () => {
				if (!(!await ne() || e)) {
					...
					e || (u(!0), Ee(), je(), De(1e3), Me(), ke(15e3), he.current = window.setInterval(() => {
						we();
					}, 3e4));
				}
			})(), () => {
				e = !0, Oe(), Ae(), he.current && clearInterval(he.current), pe.current && T.removeChannel(pe.current), O.current && T.removeChannel(O.current);
			};
		}, [
			Ee,
			we,
		...
		}, []), t ? /* @__PURE__ */ (0, n.jsxs)(n.Fragment, { children: [/* @__PURE__ */ (0, n.jsxs)("div", {
			className: "dashboard",
			children: [
				/* @__PURE__ */ (0, n.jsx)("header", { children: /* @__PURE__ */ (0, n.jsxs)("div", { children: [/* @__PURE__ */ (0, n.jsxs)("h2", {
					className: "greeting",
					children: [
						"Welcome, ",
					...
					children: "This is your project dashboard."
				})] }) }),
				/* @__PURE__ */ (0, n.jsx)("div", {
					id: "statusBar",
					className: `status-bar ${g.error ? "error" : ""}`,
					"aria-live": "polite",
					style: { display: g.text ? "" : "none" },
					children: g.text
				}),
					...
					children: [
						/* @__PURE__ */ (0, n.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, n.jsx)("div", {
								...
								children: ye.peopleInside
							}), /* @__PURE__ */ (0, n.jsx)("div", {
								className: "stat-label",
								children: "People Inside Now"
							})]
						}),
						/* @__PURE__ */ (0, n.jsxs)("div", {
							className: "stat",
							children: [/* @__PURE__ */ (0, n.jsx)("div", {
								className: "stat-value",
								children: ye.activeSpaces
							}), /* @__PURE__ */ (0, n.jsx)("div", {
								className: "stat-label",
								children: "Active Spaces"
							})]
						}),
						/* @__PURE__ */ (0, n.jsxs)("div", {
							className: "stat",
				...
				/* @__PURE__ */ (0, n.jsx)("section", {
					className: "main-grid main-grid-single",
					style: { marginTop: 0 },
					children: /* @__PURE__ */ (0, n.jsxs)("div", {
						className: "panel occupancy",
						children: [
							/* @__PURE__ */ (0, n.jsx)("h3", { children: "Live Occupancy" }),
							ve.length > 0 ? /* @__PURE__ */ (0, n.jsx)("div", {
								className: "occupancy-cards",
								"aria-live": "polite",
								children: ve.map((e) => {
									let t = Number(e.current_occupancy || 0), r = Number(e.capacity || 0);
									return /* @__PURE__ */ (0, n.jsxs)("div", {
										className: "occupancy-card",
										children: [/* @__PURE__ */ (0, n.jsxs)("div", {
											className: "occupancy-top",
											children: [/* @__PURE__ */ (0, n.jsxs)("div", { children: [/* @__PURE__ */ (0, n.jsx)("div", {
												className: "occupancy-name",
												children: e.name || e.id
											}), e.location ? /* @__PURE__ */ (0, n.jsx)("div", {
												className: "meta",
												children: e.location
											}) : null] }), /* @__PURE__ */ (0, n.jsx)("div", {
												className: "occupancy-status",
												children: a(t, r)
											})]
										}), /* @__PURE__ */ (0, n.jsxs)("div", {
											className: "occupancy-value-row",
											children: [/* @__PURE__ */ (0, n.jsx)(p, { value: Math.max(0, t) }), /* @__PURE__ */ (0, n.jsxs)("div", {
												className: "capacity-suffix",
												children: ["/ ", Math.max(0, r)]
											})]
										})]
									}, e.id);
								})
							...
							/* @__PURE__ */ (0, n.jsxs)("div", {
								className: "occupancy-sample",
								"aria-hidden": ve.length > 0 ? "true" : "false",
								style: { display: ve.length > 0 ? "none" : "" },
								children: [/* @__PURE__ */ (0, n.jsx)("p", {
									className: "small-muted",
									style: { marginTop: "12px" },
									children: ee || "No active resources. Showing sample data below."
								}), /* @__PURE__ */ (0, n.jsx)("div", {
									className: "occupancy-cards",
									"aria-hidden": "true",
									children: l().map((e) => /* @__PURE__ */ (0, n.jsxs)("div", {
										className: "occupancy-card",
										children: [/* @__PURE__ */ (0, n.jsxs)("div", {
											className: "occupancy-top",
											children: [/* @__PURE__ */ (0, n.jsxs)("div", { children: [/* @__PURE__ */ (0, n.jsx)("div", {
											...
											className: "occupancy-value-row",
											children: [/* @__PURE__ */ (0, n.jsx)(p, { value: e.current }), /* @__PURE__ */ (0, n.jsxs)("div", {
												className: "capacity-suffix",
												children: ["/ ", e.cap]
											})]
										})]
				...
				/* @__PURE__ */ (0, n.jsxs)("section", {
					...
					children: [/* @__PURE__ */ (0, n.jsxs)("div", {
						...
						children: [
							/* @__PURE__ */ (0, n.jsx)("h3", { children: "Quick Actions" }),
							/* @__PURE__ */ (0, n.jsxs)("div", {
								...
								children: [
									/* @__PURE__ */ (0, n.jsx)("button", {
										...
										children: "View Map / Resource List"
									}),
									/* @__PURE__ */ (0, n.jsx)("button", {
										type: "button",
										className: "btn btn-secondary",
							...
							/* @__PURE__ */ (0, n.jsx)("ul", {
								className: "alerts",
								children: xe.length === 0 ? /* @__PURE__ */ (0, n.jsx)("li", {
									className: "small-muted",
									children: "No alerts"
								}) : xe.map((e) => /* @__PURE__ */ (0, n.jsx)("li", {
									className: e.type === "announcement" ? "alert-item alert-announcement" : "alert-item alert-system",
									children: e.type === "announcement" ? /* @__PURE__ */ (0, n.jsxs)(n.Fragment, { children: [
										/* @__PURE__ */ (0, n.jsx)("span", {
										...
										/* @__PURE__ */ (0, n.jsx)("div", {
											...
											children: e.title || "Notice"
										}),
										e.message ? /* @__PURE__ */ (0, n.jsx)("div", {
											className: "alert-text",
											children: e.message
										}) : null,
										e.when ? /* @__PURE__ */ (0, n.jsx)("div", {
											className: "alert-meta",
											children: e.when
										}) : null
									] }) : /* @__PURE__ */ (0, n.jsx)("div", {
										className: "alert-text",
										children: e.message || e.text || e.title
									})
								}, e.id || `${e.type}-${e.message}`))
							}),
							/* @__PURE__ */ (0, n.jsxs)("div", {
								style: {
									marginTop: "8px",
								...
								children: [/* @__PURE__ */ (0, n.jsx)("button", {
									type: "button",
									className: "btn",
									style: { display: g.error ? "inline-block" : "none" },
									onClick: Ee,
									children: "Retry"
								}), /* @__PURE__ */ (0, n.jsx)("div", {
									className: "small-muted",
						...
						children: [/* @__PURE__ */ (0, n.jsxs)("div", {
							className: "activity-header-row",
							children: [/* @__PURE__ */ (0, n.jsx)("h3", { children: "Recent RFID Taps" }), /* @__PURE__ */ (0, n.jsx)("button", {
								type: "button",
								className: "activity-toggle",
								onClick: () => ue((e) => !e),
								children: se ? "Hide activity" : "Show activity"
							})]
						}), se ? /* @__PURE__ */ (0, n.jsxs)(n.Fragment, { children: [
							/* @__PURE__ */ (0, n.jsx)("ul", {
								className: "activity",
								children: Se.map((e) => {
									let t = e.resource?.name || e.resource_id || "Unknown", r = e.resource?.location || "";
									return /* @__PURE__ */ (0, n.jsxs)("li", { children: [/* @__PURE__ */ (0, n.jsxs)("div", { children: [
										/* @__PURE__ */ (0, n.jsx)("strong", { children: "RFID Tap" }),
										" · ",
										t,
									...
									] }), /* @__PURE__ */ (0, n.jsx)("div", { children: /* @__PURE__ */ (0, n.jsx)("span", {
										className: "meta",
										children: s(e.scanned_at)
									}) })] }, e.id);
								})
							}),
							/* @__PURE__ */ (0, n.jsx)("div", {
								className: "activity-footer",
								children: w.length > r ? /* @__PURE__ */ (0, n.jsx)("button", {
									type: "button",
									className: "activity-toggle",
									onClick: () => fe((e) => !e),
									children: de ? "Show less" : "View all"
								}) : null
							}),
							w.length === 0 ? /* @__PURE__ */ (0, n.jsx)("p", {
								className: "small-muted",
								children: ae || "No RFID taps yet."
							}) : null
						] }) : null]
					})]
				})
			...
			children: /* @__PURE__ */ (0, n.jsxs)("div", {
				...
				"aria-live": "polite",
				children: [
					/* @__PURE__ */ (0, n.jsx)("div", { className: "dot" }),
					/* @__PURE__ */ (0, n.jsx)("div", { className: "dot" }),
					/* @__PURE__ */ (0, n.jsx)("div", { className: "dot" }),
					/* @__PURE__ */ (0, n.jsx)("div", {
	...
	function h() {
		let e = document.getElementById("dashboardReactRoot");
		e && (0, t.createRoot)(e).render(/* @__PURE__ */ (0, n.jsx)(m, {}));
	}
