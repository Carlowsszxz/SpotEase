...
async function sha256Hex(text) {
  const bytes = new TextEncoder().encode(String(text || '').trim().toUpperCase());
...
function DashboardPage() {
  ...
  const loadRecentActivity = useCallback(async () => {
    try {
      ...
      if (!currentUser) {
        setActivityItems([]);
        setActivityError('Not logged in.');
        return;
      }
      ...
      if (!userTags || userTags.length === 0) {
        setActivityItems([]);
        setActivityError('No RFID tags registered.');
        return;
      }

      const tagHashes = await Promise.all(
        userTags
          .map((t) => t.tag_uid_norm || t.tag_uid)
          .filter(Boolean)
          .map((tagUid) => sha256Hex(tagUid))
      );
      ...
      const { data: rfidScans, error: scansErr } = await supabase
        .from('rfid_scans')
      ...
      if (scansErr) {
        setActivityItems([]);
        setActivityError('RFID activity is not available.');
        return;
      }

      const resourceIds = Array.from(new Set((rfidScans || []).map((scan) => scan.resource_id).filter(Boolean)));
      ...
      const hydrated = (rfidScans || []).map((scan) => ({
        id: scan.resource_id + '_' + scan.scanned_at,
        resource_id: scan.resource_id,
        scanned_at: scan.scanned_at,
        resource: scan.resource_id ? resourcesById.get(scan.resource_id) : null,
      }));
              ...
              <>
                <ul className="activity">
                  {visibleActivity.map((item) => {
                    const resourceName = item.resource?.name || item.resource_id || 'Unknown';
                    const location = item.resource?.location || '';

                    return (
                      <li key={item.id}>
                        <div>
                          <strong>RFID Tap</strong> · {resourceName}
                          {location ? <div className="meta">{location}</div> : null}
                        </div>
                        <div>
                          <span className="meta">{formatWhen(item.scanned_at)}</span>
                        </div>
                      </li>
                    );
                  })}
                ...
                {activityItems.length === 0 ? (
                  <p className="small-muted">{activityError || 'No RFID taps yet.'}</p>
                ) : null}
              </>
            ) : null}
          </div>
