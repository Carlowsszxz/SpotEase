...
function RollDigit({ digit }) {
  const [renderDigit, setRenderDigit] = useState(() => clampDigit(digit));
  const [animate, setAnimate] = useState(true);
  const previousDigitRef = useRef(clampDigit(digit));

  useEffect(() => {
    const nextDigit = clampDigit(digit);
    ...
    setAnimate(false);
    setRenderDigit(previousDigit);

    rafA = window.requestAnimationFrame(() => {
      rafB = window.requestAnimationFrame(() => {
        const targetIndex = nextDigit < previousDigit ? nextDigit + 10 : nextDigit;
        setAnimate(true);
        setRenderDigit(targetIndex);
        previousDigitRef.current = nextDigit;
  ...
  return (
    <span className="roll-col" data-digit={digit}>
      <span
        ...
        style={{
          transform: `translateY(-${renderDigit}em)`,
          transition: animate ? 'transform 520ms cubic-bezier(.22, 1, .36, 1)' : 'none',
        }}
      >
        {Array.from({ length: 20 }, (_, index) => (
          <span className="roll-digit" key={index}>{index % 10}</span>
        ))}
      </span>
    </span>
  );
...
function RollingNumber({ value, minDigits = 2 }) {
  ...
  safeValue = Math.max(0, Math.floor(safeValue));

  const digits = String(safeValue).split('').map((digit) => Number(digit));
  const [slotCount, setSlotCount] = useState(() => Math.max(minDigits, digits.length));

  useEffect(() => {
    setSlotCount((current) => Math.max(current, minDigits, digits.length));
  }, [digits.length, minDigits]);

  const slots = useMemo(() => {
    const leading = Math.max(0, slotCount - digits.length);
    return [...Array.from({ length: leading }, () => null), ...digits];
  }, [digits, slotCount]);

  return (
    <div className="roll-number" aria-label={String(safeValue)}>
      {slots.map((digit, index) => (
        <span
          className={`roll-digit-slot ${digit == null ? 'is-leading-empty' : ''}`}
          key={`slot-${index}`}
        >
          <RollDigit digit={digit ?? 0} />
        </span>
      ))}
    </div>
  );
...
function MapPage() {
  ...
  const includeUpdatedAtRef = useRef(false);
    ...
    try {
      ...
      if (includeUpdatedAtRef.current) {
        ...
        const isMissingUpdatedAt =
          code === '42703' ||
          (/updated_at/i.test(message) && /(does not exist|column|schema cache)/i.test(message));
        ...
        {hasValidFilters && resources.length > 0 ? (
          <section className="map-stats" aria-label="Map quick stats">
            ...
            <div className="stat-box free">
              <div>Available</div>
              <RollingNumber value={stats.available} />
            </div>
            <div className="stat-box reserved">
              <div>Reserved</div>
              <RollingNumber value={stats.reserved} />
            </div>
            <div className="stat-box occupied">
              <div>Occupied</div>
              <RollingNumber value={stats.occupied} />
            </div>
          </section>
        ) : null}
          ...
          <section className="resources-section">
            <div className="resources-list" role="list">
              {resources.map((resource) => {
                const availability = availabilityFor(resource, reservedSet);
                const currentOccupancy = Number(resource.current_occupancy || 0);
                const capacity = Number(resource.capacity || 0);
                ...
                return (
                  <article className="resource-row" role="listitem" key={resource.id}>
                    ...
                    <div className="resource-right">
                      ...
                      <p className="resource-occupancy">
                        Current occupancy: {currentOccupancy}{capacity > 0 ? ` / ${capacity}` : ''}
                      </p>
