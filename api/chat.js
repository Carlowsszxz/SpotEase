...
async function callOllama(messages) {
  ...
  try {
    ...
    return { reply: content, provider: 'ollama', model: OLLAMA_MODEL };
  } finally {
    clearTimeout(timer);
  }
}
...
async function callHuggingFaceModel(modelName, messages) {
  ...
  try {
    ...
    return { reply: generated.trim(), provider: 'huggingface', model: modelName };
  } finally {
    clearTimeout(timer);
  }
}
...
async function callHuggingFace(messages) {
  return callHuggingFaceModel(HF_MODEL, messages);
}
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'OPTIONS') {
    ...
    res.status(204).setHeader('Access-Control-Allow-Origin', origin);
  ...
  if (req.method === 'GET' && pathname === '/api/chat') {
    return json(res, 200, { ok: true, provider: PROVIDER });
  }
  ...
  try {
    ...
    if (containsBlockedContent(latestText)) {
      return json(res, 400, { error: 'Message blocked by safety filter.' });
    }
